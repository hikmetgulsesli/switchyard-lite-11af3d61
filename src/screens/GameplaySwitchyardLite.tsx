...
export interface GameplaySwitchyardLiteProps {
  ...
  runtime?: { player?: { lane?: number; position?: number }; obstacles?: Array<{ lane?: number; position?: number }>; shards?: Array<{ lane?: number; position?: number }>; score?: number; energy?: number; lives?: number; routesCompleted?: number; level?: number; paused?: boolean; status?: string };

}

export function GameplaySwitchyardLite({ actions, runtime }: GameplaySwitchyardLiteProps) {
  const score = (runtime?.score ?? 12450).toLocaleString("en-US");
  const multiplier = `x${(1 + (runtime?.routesCompleted ?? 7) * 0.5).toFixed(1)}`;
  const level = String(runtime?.level ?? 8).padStart(2, "0");
  const status = (runtime?.status ?? (runtime?.paused ? "paused" : "playing")).toUpperCase();
  ...
  return (
    <>
      ...
      <span className="text-technical-lg font-technical-lg text-primary-fixed">{score}</span>
      </div>
      <div className="flex flex-col items-center">
      <span className="text-label-caps font-label-caps text-on-surface-variant opacity-80">MULT</span>
      <span className="text-technical-lg font-technical-lg text-tertiary-fixed">{multiplier}</span>
      </div>
      <div className="flex flex-col items-center">
      <span className="text-label-caps font-label-caps text-on-surface-variant opacity-80">LVL</span>
      <span className="text-technical-lg font-technical-lg text-on-surface">{level}</span>
      </div>
      </div>
      <div className="flex items-center gap-4">
      ...
      <span className="text-label-caps font-label-caps">STATUS: {status}</span>
      </a>
      ...
      <span className="text-label-caps font-label-caps">SCORE: {score}</span>
      </a>
      ...
      <span className="text-label-caps font-label-caps">MULT: {multiplier}</span>
      </a>
      ...
      <span className="text-label-caps font-label-caps">LVL: {level}</span>
      </a>
      </nav>
      ...
      <span className="text-technical-md font-technical-md text-primary-fixed">{score}</span>
      </div>
      <div className="flex flex-col items-center">
      <span className="text-label-caps font-label-caps text-on-surface-variant">MULT</span>
      <span className="text-technical-md font-technical-md text-tertiary-fixed">{multiplier}</span>
      </div>
      <div className="flex flex-col items-end">
      <span className="text-label-caps font-label-caps text-on-surface-variant">LVL</span>
      <span className="text-technical-md font-technical-md text-on-surface">{level}</span>
      </div>
      </div>
